...
const RepaymentPage = () => {
  ...
  // If slug provided but neither lender nor city found â 404
  if (slug && !lender && !city) return <Navigate to="/calculators/repayment" replace />;
  ...
  const seoTitle = city
    ? `${city.name} Mortgage Calculator â Monthly Repayments | RepayWise`
    : lender
    ? `${lender.name} Mortgage Calculator â Free UK Tool | RepayWise`
    : "Mortgage Repayment Calculator â Free UK Tool | RepayWise";
              ...
              mainEntity: [
                {
                  ...
                  acceptedAnswer: { "@type": "Answer", text: "UK mortgage repayments are calculated using the standard amortisation formula. The monthly payment equals P ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ r ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ (1+r)^n / ((1+r)^n ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ 1), where P is the loan amount, r is the monthly interest rate, and n is the total number of monthly payments." },
                },
                {
                  "@type": "Question",
            ...
            >
              ...
              <div className="space-y-5">
                <BigSlider
                  ...
                  prefix="ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ£"
                  value={propertyPrice}
                  min={50_000}
                  max={2_500_000}
                  step={5_000}
                ...
                <div>
                  <div className="flex items-center justify-end mb-2">
                    <div className="inline-flex rounded-lg border border-border bg-background p-0.5 text-[10px] font-bold uppercase tracking-wider">
                      ...
                      >
                        ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ£ Amount
                      </button>
                      <button
                        type="button"
                ...
                <div className="grid sm:grid-cols-[1fr_auto] gap-4 items-center rounded-2xl border border-border p-4 bg-secondary/40">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1 tabular-nums">
                      LTV {ltv.toFixed(1)}% ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· Deposit {depositPct.toFixed(1)}%
                    </p>
                  </div>
            ...
            <StepPane eyebrow="Your numbers" question="Here is what it costs">
              <div className="space-y-4">
                <div className="rounded-2xl bg-primary text-primary-foreground p-6 shadow-glow-cyan">
                  ...
                  <div className="mt-4 pt-4 border-t border-primary-foreground/10 flex flex-wrap gap-x-8 gap-y-2 text-xs">
                    <span className="text-primary-foreground/60">
                      If rates rise +1% ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ{" "}
                      <span className="text-primary-foreground font-semibold">
                        {formatGBP(stressed.monthlyPayment, { decimals: 2 })}/mo
                      </span>
                    </span>
                    <span className="text-primary-foreground/60">
                      Total interest ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ{" "}
                      <span className="text-primary-foreground font-semibold">
                    ...
                    <div className="h-44">
                      <ResponsiveContainer>
                        <AreaChart data={chart} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
                          ...
                          <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} tickFormatter={(value) => `ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ£${(value / 1000).toFixed(0)}k`} />
                          <Tooltip
                ...
                <ShareCalculation
                  ...
                  intro={`Property ${formatGBP(propertyPrice)} ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· Deposit ${formatGBP(deposit)} (${depositPct.toFixed(1)}%)`}
                  summary={[
