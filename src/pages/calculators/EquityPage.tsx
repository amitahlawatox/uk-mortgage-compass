...
const EquityPage = () => {
  ...
  const refinanceBand =
    ltvNow <= 60 ? "60% LTV Ã¢ÂÂ best rates"
    : ltvNow <= 75 ? "75% LTV Ã¢ÂÂ competitive rates"
    : ltvNow <= 85 ? "85% LTV Ã¢ÂÂ standard rates"
    : ltvNow <= 90 ? "90% LTV Ã¢ÂÂ limited choice"
    : "Above 95% LTV Ã¢ÂÂ very few products";
  ...
  return (
    ...
    >
      <SEO
        title="Home Equity Calculator UK Ã¢ÂÂ Refinance, Sell or Remortgage"
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="glass-card rounded-2xl p-6 space-y-5">
            ...
            <SliderField label="Purchase price (when bought)" prefix="ÃÂ£" value={purchasePrice} min={50_000} max={2_000_000} step={5_000} onChange={setPurchasePrice} />
            <SliderField label="Current market value" prefix="ÃÂ£" value={currentValue} min={50_000} max={3_000_000} step={5_000} onChange={setCurrentValue} />
          ...
          <div className="glass-card rounded-2xl p-6 space-y-5">
            ...
            <SliderField label="Monthly overpayment made" prefix="ÃÂ£" value={monthlyOver} min={0} max={2_000} step={25} onChange={setMonthlyOver} />
            <SliderField label="Lump-sum overpayment made" prefix="ÃÂ£" value={lumpSum} min={0} max={200_000} step={500} onChange={setLumpSum} />
            {lumpSum > 0 && (
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="glass-card rounded-2xl p-6 bg-gradient-to-br from-accent-secondary/15 to-transparent border-accent-secondary/30">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {equityPct.toFixed(1)}% of current value ÃÂ· Current LTV {ltvNow.toFixed(1)}%
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="glass-card rounded-2xl p-5 space-y-3">
              ...
              <div className="space-y-1.5 text-sm">
                ...
                <Row label="Pay off mortgage" value={`Ã¢ÂÂ ${formatGBP(outstanding)}`} />
                <Row label={`Selling costs (${sellingCostsPct.toFixed(1)}%)`} value={`Ã¢ÂÂ ${formatGBP(sellingCosts)}`} />
          ...
          <ShareCalculation
            ...
            calculator="equity"
            intro={`Bought ${formatGBP(purchasePrice)} ÃÂ· Now worth ${formatGBP(currentValue)} ÃÂ· ${yearsOwned}y owned`}
            summary={[
