  ...
  return (
    ...
    >
      <SEO
        ...
        calculatorType="Mortgage Overpayment Calculator"
      />
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        <div className="lg:col-span-2 glass-card rounded-2xl p-6 space-y-5">
          ...
          <SliderField label="Property price" prefix="ÃÂÃÂ£" value={propertyPrice} min={50_000} max={2_500_000} step={5_000} onChange={handlePropertyChange} />
          <DepositField value={deposit} onChange={handleDepositChange} referencePrice={propertyPrice} />
          <SliderField label="Loan amount (override)" prefix="ÃÂÃÂ£" value={principal} min={0} max={2_500_000} step={5_000} onChange={setPrincipal} />
          ...
          <SliderField label="Monthly overpayment" prefix="ÃÂÃÂ£" value={monthlyOver} min={0} max={2000} step={25} onChange={setMonthlyOver} />
          <SliderField label="Quarterly overpayment" prefix="ÃÂÃÂ£" value={quarterlyOver} min={0} max={6000} step={50} onChange={setQuarterlyOver} />
          <SliderField label="Annual overpayment" prefix="ÃÂÃÂ£" value={annualOver} min={0} max={25_000} step={100} onChange={setAnnualOver} />
          <SliderField label="One-off lump sum" prefix="ÃÂÃÂ£" value={lumpSum} min={0} max={100_000} step={500} onChange={setLumpSum} />
          {lumpSum > 0 && (
        ...
        <div className="lg:col-span-3 space-y-4">
            ...
            <div className="h-72">
              <ResponsiveContainer>
                <AreaChart data={chart} margin={{ top: 5, right: 8, left: -8, bottom: 0 }}>
                  ...
                  <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }} tickFormatter={(value) => `ÃÂÃÂ£${(value / 1000).toFixed(0)}k`} />
                  <Tooltip
          ...
          {monthlyOver > 0 || lumpSum > 0 ? (
            <div className="rounded-2xl bg-primary text-primary-foreground p-5 sm:p-6 shadow-glow-cyan">
              ...
              <p className="mt-4 text-[11px] text-primary-foreground/60 leading-relaxed">
                Based on your inputs, every ÃÂÃÂ£1 overpaid saves roughly{" "}
          ...
          <ShareCalculation
            ...
            intro={`Loan ${formatGBP(principal)} ÃÂÃÂ· ${rate.toFixed(2)}% ÃÂÃÂ· ${term} years`}
            summary={[
