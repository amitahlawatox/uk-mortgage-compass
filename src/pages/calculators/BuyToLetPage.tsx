...
const BuyToLetPage = () => {
  ...
  // BTL is an "additional property" ÃÂ¢ÃÂÃÂ surcharge applies
  const stamp = useMemo(
  ...
  // ICR = rent / interest portion (lender stress test, usually ÃÂ¢ÃÂÃÂ¥125% @ stressed rate)
  ...
  return (
    ...
    >
      <SEO
        title="Buy-to-Let Mortgage Calculator UK ÃÂ¢ÃÂÃÂ Yield, Stamp Duty & EMI"
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="glass-card rounded-2xl p-6 space-y-5">
            ...
            <SliderField label="House price" prefix="ÃÂÃÂ£" value={housePrice} min={50_000} max={2_000_000} step={5_000} onChange={(v) => {
              setHousePrice(v);
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-semibold tabular-nums">{formatGBP(loan)} ÃÂÃÂ· LTV {ltv.toFixed(1)}%</span>
            </div>
          </div>
          ...
          <div className="glass-card rounded-2xl p-6 space-y-5">
            ...
            <SliderField label="Expected monthly rent" prefix="ÃÂÃÂ£" value={monthlyRent} min={200} max={10_000} step={50} onChange={setMonthlyRent} />
            <SliderField label="Monthly costs (mgmt, insurance, maintenance)" prefix="ÃÂÃÂ£" value={monthlyCosts} min={0} max={2_000} step={10} onChange={setMonthlyCosts} />
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="glass-card rounded-2xl p-6 bg-gradient-to-br from-accent-secondary/15 to-transparent border-accent-secondary/30">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {repayType === "interest-only" ? "Interest only" : "Capital + interest"} ÃÂÃÂ· {term}y ÃÂÃÂ· {rate.toFixed(2)}% APR ÃÂÃÂ· LTV {ltv.toFixed(1)}%
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="glass-card rounded-2xl p-5 space-y-2 text-sm">
              ...
              <p className={`text-[11px] mt-1 ${icr >= 125 ? "text-accent-secondary" : "text-destructive"}`}>
                {icr >= 125 ? "Meets typical 125% lender ICR threshold." : "Below typical 125% lender ICR ÃÂ¢ÃÂÃÂ may not pass."}
              </p>
            </div>
          </div>
          ...
          <div className="glass-card rounded-2xl p-5 space-y-2 text-sm">
            ...
            <Row label="Annual mortgage" value={`ÃÂ¢ÃÂÃÂ ${formatGBP(emi * 12)}`} />
            <Row label="Annual costs" value={`ÃÂ¢ÃÂÃÂ ${formatGBP(monthlyCosts * 12)}`} />
            <div className="h-px bg-border my-1" />
          ...
          <ShareCalculation
            ...
            intro={`${formatGBP(housePrice)} BTL ÃÂÃÂ· ${formatGBP(deposit)} deposit ÃÂÃÂ· ${region}`}
            summary={[
